...
In practice, we might assume a positive linear relationship between a teacher's experience and students' mean math score. However, the scatter plot suggests that there is no significant linear relationship between teaching experience and mean math score for each teacher. 

Furthermore, as the average remains consistent around 525 to 530 across teacher career levels, we may conclude that teacher career does not have linear association with students' mean math scores. 
...
For urbanicity, when school ID is classified as "inner city" math scores tend to be lower compared to other area. Further analysis is needed for this variable. Additionally, most schools are situated in rural areas. Class sizes are uniformly distributed across each school, suggesting that each school has a consistent range of class sizes. Therefore, including school ID as a variable and examining the effect of class size when it is fixed may provide insights into the impact of class size on academic performance.
...
compare_box=ggplot(combined_data, aes(x = Dataset, y = Score, fill = Dataset)) +
  ...
  scale_fill_manual(values = c("Mean Math Score (Teacher)" = "lightblue", "Mean Math Score (School)" = "lightgreen"),
                     labels = c("Mean Math Score (School)", "Mean Math Score (Teacher)")) +
...
Despite our small sample size is not enough to fully investigate larger variability in mean scores per school, further analysis will be helpful to understand which underlying factors contribute to these significant variations.


The boxplot illustrates the dispersion of **mean math scores across schools** compared to mean math scores across teachers, showing that the values are more scattered across schools (minimum of 489.4 to maximum of 570.7). 
...
Result of our analysis can be interpreted by: 
...
model_res=ggplot(data, aes(x = class_type)) +
  ...
  labs(title = "Comparison of Residuals from Two Models", x = "Class Type", y = "Residuals") +
    scale_color_manual(name = "Model", values = c("Mean model" = "red", "Median model" = "blue")) + scale_x_discrete(labels = c("1" = "small", "2" = "regular", "3" = "regular+aide"))+theme_minimal()
